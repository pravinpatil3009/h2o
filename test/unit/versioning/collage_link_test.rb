...
class CollageLinkTest < ActiveSupport::TestCase
  ...
  def test_collage_copy_returns_collage_link_for_a_version
    ...
    assert_equal 1, @collage_link.host_collage_version
    assert_equal 1, @collage_link.linked_collage_version
    assert_equal @collage_link.link_text_start, Collage.copy_by_id_and_version(first_collage.id, 1).collage_links.first.link_text_start
  end
