...
module Rails
  module Upgrading
    ...
    class RoutesUpgrader
      H2O_PATH = '/Users/timcase/Sites/h2o/'

      def self.generate_h2o_routes
      ...
      def has_routes_file?
        File.exists?(H2O_PATH + "config/routes.rb")
      end

      def routes_code
        File.read(H2O_PATH + "config/routes.rb")
      end
    ...
    class RouteRedrawer
      ...
      def connect(path, options={})
        ...
        if (path == ":controller/:action/:id.:format" || path == ":controller/:action/:id")
          #we don't want default routes in rails 4
        else
